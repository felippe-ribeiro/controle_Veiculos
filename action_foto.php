...
<?php 
		...
		$resp_cad = (isset($_POST['resp_cad'])) ? $_POST['resp_cad'] : '';
		$resp_exit = (isset($_POST['resp_exit'])) ? $_POST['resp_exit'] : '';
    ...
    // Verifica se foi solicitada a inclusão de dados
		if ($acao == 'incluir'):
			...
			$sql = 'INSERT INTO tb_cadastro (matricula, tipo, situacao, nome, identidade, placa, veiculo, cidade, uf, empresa, foto, resp_cad)VALUES(:matricula, :tipo, :situacao, :nome, :identidade, :placa, :veiculo, :cidade, :uf, :empresa, :foto, :resp_cad)';
		...
		//	$stm->bindValue(':datacadastro', $dataLocal);
			...
			$stm->bindValue(':resp_cad', $resp_cad);
          ...
          //  echo $sql;
			...
			if ($retorno):
				echo "<div class='alert alert-success' role='alert'>Cadastro Realizado com Sucesso, aguarde você está sendo redirecionado ...</div> ";
		    else:
	   ...
	   // Verifica se foi solicitada a edição de dados
		if ($acao == 'editar'):
			...
			foto=:foto, dataentrada=:dataentrada, horaentrada=:horaentrada, horasaida=:horasaida, resp_exit=:resp_exit ';
			$sql .= ' WHERE id = :id';
			...
			$stm->bindValue(':foto', $nome_foto);
			$stm->bindValue(':id', $id);
			$stm->bindValue(':resp_exit', $resp_exit);
		   ...
		   // echo $sql;
			...
			echo "<meta http-equiv=refresh content='3;URL=r_entrada.php'>";
		endif;
